...
def savecheckpoint(state, is_best, args):
    ...
    # torch.save(state, args.data + f"/checkpoint_lr={args.learning_rate}.pt")
    if is_best:
        shutil.copyfile(args.checkpoint, os.path.join(args.data, 'best_model.pt'))
        # shutil.copyfile(args.checkpoint, os.path.join(args.data, f'best_model_lr{args.learning_rate}.pt'))
...
def validate(valData, model, lossF, args):
    ...
    num_iteration = int(valData.size / args.batch)
    ...
    for iteration in range(num_iteration):
        ...
        with torch.no_grad():
            ...
            # total_accuracy += accuracy * gendata.trg.size(1)
