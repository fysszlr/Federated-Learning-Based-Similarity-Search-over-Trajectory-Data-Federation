...
def data_split(srcfile, trgfile, mtafile, num_user):
    srcdatas = [[] for _ in range(num_user)]
    trgdatas = [[] for _ in range(num_user)]
    mtadatas = [[] for _ in range(num_user)]
    srcstream, trgstream, mtastream = open(srcfile, 'r'), open(trgfile, 'r'), open(mtafile, 'r')
    totle_len = 0
    for (s, t, m) in zip(srcstream, trgstream, mtastream):
        totle_len = totle_len + 1
    len_per_clients = totle_len / num_user
    print(f"totle_len is :{totle_len}\n"
          f"len_per_clients is :{len_per_clients}")
    srcstream.seek(0, 0)
    trgstream.seek(0, 0)
    mtastream.seek(0, 0)
    for i, (s, t, m) in enumerate(zip(srcstream, trgstream, mtastream)):
        idx = int(i / len_per_clients)
        srcdatas[idx].append(s)
        trgdatas[idx].append(t)
        mtadatas[idx].append(m)
    return srcdatas, trgdatas, mtadatas
...
class DataLoader():
    ...
    def load(self, max_num_line=0):
        ...
        for (s, t, m) in zip(self.srcfile, self.trgfile, self.mtafile):
