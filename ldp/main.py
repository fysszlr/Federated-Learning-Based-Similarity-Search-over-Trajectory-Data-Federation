...
import ldp.trajectory as trajectory
import ldp.ldp as ldp
import ldp.dataset as dataset
from ldp.grid import GridMap, Grid
import ldp.map_func as map_func
from ldp.parse import args
import logging
import random
import pandas as pd
    ...
    for t in grid_db:
        ...
        for i in range(length - 1):
            ...
            if grid_map.is_adjacent_grids(curr_grid, next_grid):
                ...
                logging.info('Trajectory has non-adjacent moves, use non-adjacent map function!')
        end = t[length - 1]
...
def dataset_stats(db: List[List[Tuple[float, float]]]):
...
grid_map = GridMap(1,1,1,1,1)
def get_ldp_dataset(ori_dataset):
    db = ori_dataset.data.merc_seq.values
    print(db)
    ...
    logging.info('Convert raw trajectories to grids...')
    ...
    logging.info('Updating Markov prob...')
    ...
    return TrajDataset(pd.DataFrame({'merc_seq': synthetic_trajectories}))

    # ============================ EXPERIMENTS =========================== #
    # np.random.seed(2022)
    # random.seed(2022)
    # logging.info('Experiment: Density Error...')
    ...
    # logging.info(f'Density Error: {density_error}')
    #
    # logging.info('Experiment: Hotspot Query Error...')
    # hotspot_ndcg = experiment.calculate_hotspot_ndcg(orig_density, syn_density)
    # logging.info(f'Hotspot Query Error: {1-hotspot_ndcg}')
    # # Query AvRE
    # logging.info('Experiment: Query AvRE...')
    #
    ...
    #                                                queries)
    # logging.info(f'Point Query AvRE: {query_error}')
    #
    # # Location coverage Kendall-tau
    # logging.info('Experiment: Kendall-tau...')
    # kendall_tau = experiment.calculate_coverage_kendall_tau(orig_grid_trajectories,
    #                                                         synthetic_grid_trajectories,
    #                                                         grid_map)
    # logging.info(f'Kendall_tau:{kendall_tau}')
    #
    # # Trip error
    # logging.info('Experiment: Trip error...')
    ...
    # logging.info(f'Trip error: {trip_error}')
    #
    # # Diameter error
    # logging.info('Experiment: Diameter error...')
    # diameter_error = experiment.calculate_diameter_error(orig_trajectories, synthetic_trajectories,
    #                                                      multi=args.multiprocessing)
    # logging.info(f'Diameter error: {diameter_error}')
    #
    # # Length error
    # logging.info('Experiment: Length error...')
    # length_error = experiment.calculate_length_error(orig_trajectories, synthetic_trajectories)
    # logging.info(f'Length error: {length_error}')
    #
    # # Pattern mining errors
    # logging.info('Experiment: Pattern mining errors...')
    ...
    #
    # logging.info(f'Pattern F1 error: {pattern_f1_error}')
    # logging.info(f'Pattern support error: {pattern_support_error}')
    #
